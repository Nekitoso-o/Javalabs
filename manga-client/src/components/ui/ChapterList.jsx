                        ...
                        {files?.length > 0 && (
                            <div className="chapter-files-preview">
                                {Array.from(files).slice(0, 8).map((f, i) => (
                                    <div key={i} className="chapter-files-preview__item">
...
export default function ChapterList({ comicId, editMode = false }) {
    ...
    const [confirmDelete, setConfirmDelete] = useState(null); // { chapter }
    ...
    const load = useCallback(() => {
        comicChaptersApi.getAll(comicId).then(setChapters).catch(() => {});
    }, [comicId]);
            ...
            {chapters.length === 0 ? (
                ...
                <div className="ch-items">
                    {sorted.map((ch) => (
                        <div
                            key={ch.id}
                            className={`ch-item${loadingChapter === ch.id ? ' ch-item--loading' : ''}`}
                    ...
                    >
                        ...
                        <div className="chapter-modal__body">
                            ...
                            <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 20 }}>
                                Будут удалены все {confirmDelete.pageCount} страниц.
